...
(define (win? board p)
  (let ((futures (list
                  (future (check-row board p 1))
                  (future (check-row board p 2))
                  (future (check-row board p 3))
                  (future (check-col board p 1))
                  (future (check-col board p 2))
                  (future (check-col board p 3))
                  (future (check-diag1 board p))
                  (future (check-diag2 board p)))))
    (let check-futs ((l futures))
      (if (pair? l)
          (if (deref (car l))
              (check-futs (cdr l))
              #f)
