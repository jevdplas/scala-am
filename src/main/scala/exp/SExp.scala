...
/**
  * A quasiquoted element, such as `foo, `(foo (bar)), etc.
  */
...
/**
  * An unquoted element, such as `(foo ,(bar)), etc.
  */
...
/**
  * A spliced element, such as `(foo ,@(bar)), etc.
  */
case class SExpSpliced(content: SExp, pos: Position) extends SExp {
  override def toString = s",@$content"
}