...
class Recorder[TID, Addr, Abs]{
    ...
    class Recorded[V](val recordName: String, val toStr: V => String = (v: V) => v.toString) {
        ...
        def record(t: TID, v: V) = content = content + (t -> (content(t) + v))
    ...
    def outputRecorded(file: String, main: TID): Unit = {
        def keys = recordedCreate.keys ++ recordedJoin.keys ++ recordedRead.keys ++ recordedWrite.keys
        val p = main.toString.r
        val fw = new FileWriter("./recordings/" ++ file)
        try {
            keys.foreach(k =>
                fw.append(p.replaceAllIn(s"($k (${recordedCreate.report(k)} ${recordedJoin.report(k)} ${recordedRead.report(k)} ${recordedWrite.report(k)}))\n".filter(_ != '#'), "main")))
        } catch {
