...
class IncAtom[Exp <: Expression, A <: Address, V, T, TID <: ThreadIdentifier](
      ...
      iState.work.foldLeft(iState.copy(work = List())) {
        case (iStateAcc, curState) =>
          if (iStateAcc.visited.contains(curState)) iStateAcc
            ...
            val StepResult(successors, created, result, effects, store, kstore) =
              ...
              case (acc @ (r, w, j), eff) =>
                eff match {
                  ...
                  case WriteAddrEff(addr: A @unchecked) =>
                    (r, w + (addr -> (w(addr) + curState)), j)
                  case _ => acc
                }
            }
            val vis =
              if (store.asInstanceOf[DeltaStore[A, V]].updated.nonEmpty || kstore
            ...
            InnerLoopState(
              ...
              store.asInstanceOf[DeltaStore[A, V]].clearUpdated,
              kstore.asInstanceOf[DeltaStore[KAddr, Set[Kont]]].clearUpdated,
              iStateAcc.results,
              vis,
          ...
          val todoEffects: List[State] = iState.deps.written.keySet.foldLeft(List[State]())(
            (acc, addr) =>
              if (oStateAcc.store.lookup(addr) == iState.store.lookup(addr)) acc
                ...
                acc ++ readDeps(addr).filter(_.tid != stid) ++ writeDeps(addr).filter(_.tid != stid)
          )
  ...
  override def run[G](program: Exp, timeout: Timeout.T)(
    ...
    Graph[G, State, Transition].empty
      .addEdges(findConnectedStates(timeout, result.threads.values.flatten.toList, result.edges))

  }

  def runWithLabels[G](program: Exp, timeout: Timeout.T)(
      implicit ev: Graph[G, State, Transition]
  ): G = {
