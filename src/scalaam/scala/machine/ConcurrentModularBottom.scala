...
class ConcurrentModularBottom[Exp <: SchemeExp, A <: Address, V, T, TID <: ThreadIdentifier](val t: StoreType, val sem: Semantics[Exp, A, V, T, Exp], val allocator: TIDAllocator[TID, T, Exp])(
    ...
    case class State(tid: TID, control: Control, cc: KAddr, time: T, kstore: KStore) extends GraphElement with SmartHash {
        ...
        private def act(action: Act, time: T, old: VStore, cc: KAddr, results: RetVals): StepResult = action match {
            ...
            case DerefFuture(ftid: TID@unchecked, store, effs) =>
                StepResult(Set(State(tid, ControlKont(results(ftid)), cc, timestamp.tick(time), kstore)), Set.empty, Set(ftid), Option.empty, effs ++ Effects.join(ftid), store)
