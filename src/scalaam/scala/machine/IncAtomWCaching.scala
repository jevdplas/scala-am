...
class IncAtomWCaching[Exp <: Expression, A <: Address, V, T, TID <: ThreadIdentifier](
...
)(override implicit val timestamp: Timestamp[T, Exp], override implicit val lattice: Lattice[V])
  ...
  override def run[G](program: Exp, timeout: Timeout.T)(
      implicit ev: Graph[G, State, Transition]
  ): G = {

    /** Class collecting the dependencies of all threads. */
    case class Deps(
        joined: ExtendedStateJoinDeps,
        read: ExtendedStateReadDeps,
        written: ExtendedStateWriteDeps
    )
    ...
    def innerLoop(iState: InnerLoopState): InnerLoopState = {
      ...
      innerLoop(iState.work.foldLeft(iState.copy(work = List())) {
        case (iStateAcc, curState) =>
          if (iStateAcc.visited.contains(curState)) iStateAcc
          else {
            val StepResult(successors, created, result, effects, store: TStore, kstore: TKStore) =
            ...
            // empty visited set.
            val (read, written, joined) = effects.foldLeft(
              (iStateAcc.deps.read, iStateAcc.deps.written, iStateAcc.deps.joined)
            ) {
              case (acc @ (r, w, j), eff) =>
                eff match {
                    ...
                    (
                      ...
                      j + (tid -> (j(tid) + (
                        (
                          curState,
                          iStateAcc.visited,
                          iStateAcc.store.version,
                          iStateAcc.kstore.version
                        )
                      )))
                    )
            ...
            val vis =
              if (store.version != iStateAcc.store.version || kstore.version != iStateAcc.kstore.version)
                Set.empty[State]
              else iStateAcc.visited + curState
            InnerLoopState(
              iStateAcc.work ++ successors,
              store,
              kstore,
              iStateAcc.results,
              vis,
              lattice.join(iStateAcc.result, result.getOrElse(lattice.bottom)),
              iStateAcc.created ++ created,
              iStateAcc.effects ++ effects,
    ...
    def outerLoop(oState: OuterLoopState, iteration: Int = 1): OuterLoopState = {
      ...
      outerLoop(
        oState.work.foldLeft(oState.copy(work = List())) {
          case (oStateAcc, (curState, prevVisisted, version, kstoreVersion)) =>
            ...
            val iState = innerLoop(
              InnerLoopState(
                ...
                oStateAcc.kstore,
                oStateAcc.results,
                if (version == oStateAcc.store.version || kstoreVersion != oStateAcc.kstore.version)
                  prevVisisted
                else Set.empty
              )
            )
            // Some new threads may have been spawned.
            val (todoCreated, newThreads): (Set[WorkItem], Threads) =
              iState.created.foldLeft((Set[WorkItem](), oStateAcc.threads)) {
                case ((createdAcc, threadsAcc), curState) =>
                  ...
                  // A new thread will be evaluated with an empty visited set anyway, so add store version number 0.
                    ...
                    (
                      createdAcc + ((curState, Set.empty, 0, 0)),
            ...
            val todoEffects: List[WorkItem] = iState.deps.written.keySet.foldLeft(List[WorkItem]())(
              (acc, addr) =>
                if (oStateAcc.store.lookup(addr) == iState.store.lookup(addr)) acc
                else
                  acc ++ readDeps(addr).filter(_._1.tid != stid) ++ writeDeps(addr)
                    .filter(_._1.tid != stid)
            )
            ...
            val todoJoined: Set[WorkItem] =
              if (oStateAcc.results(stid) == retVal) Set.empty else joinDeps(stid)
            val fromInterference
                : Set[WorkItem] = todoJoined ++ todoEffects // Use a set to suppress duplicates.
            OuterLoopState(
              newThreads,
              oStateAcc.work ++ todoCreated ++ fromInterference,
              Deps(joinDeps, readDeps, writeDeps),
...
class TimestampedStore[A <: Address, V](
    override val content: Map[A, V],
    override val updated: Set[A],
    vs: Int = 0
  ...
  override def extend(a: A, v: V) = content.get(a) match {
    ...
    case Some(v2) =>
      new TimestampedStore[A, V](content + (a -> lat.join(v, v2)), updated + a, vs + 1)
  }
}
