...
abstract class AtomAnalysisRevDeps[Exp, A <: Address, V, T, TID <: ThreadIdentifier](
  ...
  type  JoinDeps      = Map[RestartTarget, Set[TID]]
  type  ReadDeps      = Map[RestartTarget, Set[  A]]
  type WriteDeps      = Map[RestartTarget, Set[  A]]
          ...
          .foldLeft(oState.copy(work = Set())) {
            case (oStateAcc, (stid, curStates)) =>
              ...
              val todoJoined: Set[State] =
                ...
                else convertToStates(iState.deps.joined.filter({case (_, vs) => vs.contains(stid)}).keySet, newThreads) // Could be oStateAcc.threads but would not be beneficial for precision.
