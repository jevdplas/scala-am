...
trait Semantics[Exp, Addr <: Address, V, T, C] {
    ...
    object Action {
        ...
        case class StepIn(fexp: Exp, clo: (Exp, Environment[Addr]), e: Exp, env: Environment[Addr], store: Store[Addr, V], effs: Effects = Set.empty) extends A
        case class Err(err: Error) extends A
        
        // Action used when a new future needs to be created.
        case class NewFuture[TID <: ThreadIdentifier, F](tid: TID, tidv: V, first: Exp, fram: F, env: Environment[Addr], store: Store[Addr, V], effs: Effects = Set.empty) extends A
        case class  DerefFuture[TID <: ThreadIdentifier](tid: TID,                                                       store: Store[Addr, V], effs: Effects = Set.empty) extends A
        case class                               Swap[E](                 atomExp: Exp,                                    action: A, f: Frame, effs: Effects = Set.empty) extends A
