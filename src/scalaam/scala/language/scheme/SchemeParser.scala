...
trait SchemeCompiler {
    ...
    def compile(exp: SExp): SchemeExp = exp match {
        ...
        _) =>
            SchemeNamedLet(name, this.compileBindings(bindings), this.compile(first) :: this.compileBody(rest), exp.pos)
        case SExpPair(SExpId(Identifier("let", _)),
        SExpPair(bindings, SExpPair(first, rest, _), _),
        _) =>
            SchemeLet(this.compileBindings(bindings), this.compile(first) :: this.compileBody(rest), exp.pos)
        case SExpPair(SExpId(Identifier("let", _)), _, _) =>
            throw new Exception(s"Invalid Scheme let: $exp")
        case SExpPair(SExpId(Identifier("let*", _)),
        SExpPair(bindings, SExpPair(first, rest, _), _),
        _) =>
            SchemeLetStar(this.compileBindings(bindings), this.compile(first) :: this.compileBody(rest), exp.pos)
        case SExpPair(SExpId(Identifier("let*", _)), _, _) =>
            throw new Exception(s"Invalid Scheme let*: $exp")
        case SExpPair(SExpId(Identifier("letrec", _)),
        SExpPair(bindings, SExpPair(first, rest, _), _),
        _) =>
            SchemeLetrec(this.compileBindings(bindings), this.compile(first) :: this.compileBody(rest), exp.pos)
        ...
        _) =>
            SchemeDefineFunction(name, this.compileArgs(args), this.compile(first) :: this.compileBody(rest), exp.pos)
        case SExpPair(SExpId(Identifier("do", _)),
        SExpPair(bindings, SExpPair(SExpPair(test, finals, _), commands, _), _),
        _) =>
            SchemeDo(this.compileDoBindings(bindings),
                this.compile(test),
    ...
    def compileCondClauses(clauses: SExp): List[(SchemeExp, List[SchemeExp])] = clauses match {
        ...
        _) =>
            List((SchemeValue(ValueBoolean(true), clauses.pos), this.compile(first) :: this.compileBody(rest)))
        case SExpPair(SExpPair(cond, SExpPair(first, rest, _), _), restClauses, _) =>
            (this.compile(cond), this.compile(first) :: this.compileBody(rest)) :: this.compileCondClauses(restClauses)
...
trait SchemeUndefiner {
        ...
        exps match {
            ...
            case _ :: _ =>
                if (defs.isEmpty) {
                    ...
                    tailcall(this.undefineBody(exps)).flatMap(body =>
    ...
    def undefineExpr(exp: SchemeExp): TailRec[SchemeExp] = exp match {
        ...
        case SchemeFuncall(f, args, pos) =>
            tailcall(this.undefine1(f)).flatMap(fun =>
                this.trampolineM(this.undefine1, args).map(argsv => SchemeFuncall(fun, argsv, pos)))
        case SchemeIf(cond, cons, alt, pos) =>
            tailcall(this.undefine1(cond)).flatMap(condv =>
                tailcall(this.undefine1(cons)).flatMap(consv =>
                    tailcall(this.undefine1(alt)).map(altv => SchemeIf(condv, consv, altv, pos))))
        case SchemeLet(bindings, body, pos) =>
            this.trampolineM((x: (Identifier, SchemeExp)) =>
                x match {
            ...
            this.trampolineM(
                (e: (SchemeExp, List[SchemeExp])) =>
                    e match {
                        case (cond, body) =>
                            tailcall(this.undefine1(cond)).flatMap(condv =>
                                tailcall(this.undefineBody(body)).map(bodyv => (condv, bodyv)))
                    },
                clauses
        ...
        case SchemeCase(key, clauses, default, pos) =>
            tailcall(this.undefine1(key)).flatMap(
                keyv =>
                    this.trampolineM((c: (List[SchemeValue], List[SchemeExp])) =>
                        c match {
                            case (vs, body) =>
                                tailcall(this.undefineBody(body)).map(bodyv => (vs, bodyv))
                        },
                        clauses).flatMap(clausesv =>
                        tailcall(this.undefineBody(default)).map(defaultv =>
        ...
        case SchemeOr(args, pos) => this.trampolineM(this.undefine1, args).map(argsv => SchemeOr(argsv, pos))
        case SchemeDo(vars, test, finals, commands, pos) =>
            this.trampolineM(
                (x: (Identifier, SchemeExp, Option[SchemeExp])) =>
                    x match {
                        case (id, init, step) =>
                            tailcall(this.undefine1(init)).flatMap(initv =>
                                step match {
                                    case Some(s) =>
                                        this.undefine1(s).map(stepv => {
                ...
                tailcall(this.undefine1(test)).flatMap(testv =>
                    tailcall(this.undefineBody(finals)).flatMap(finalsv =>
                        tailcall(this.undefineBody(commands).map(commandsv =>
