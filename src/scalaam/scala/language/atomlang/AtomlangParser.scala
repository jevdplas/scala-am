...
object AtomlangCompiler extends SchemeCompiler {
  ...
  override def compile(exp: SExp): SchemeExp = exp match {
    ...
    case SExpPair(SExpId(Identifier("future", _)), body, _) =>
      AtomlangFuture(this.compileBody(body), exp.pos)
...
object AtomlangRenamer extends SchemeRenamer {
  ...
  override def rename(
      exp: SchemeExp,
      names: AtomlangRenamer.NameMap,
      count: AtomlangRenamer.CountMap
  ): (SchemeExp, AtomlangRenamer.CountMap) = exp match {
...
object AtomlangUndefiner extends SchemeUndefiner {
  ...
  def undefineExpr(exp: SchemeExp): TailRec[SchemeExp] = exp match {
    ...
    case AtomlangSwap(atom, fun, args, pos) =>
      tailcall(this.undefine1(atom)).flatMap(
        atomv =>
          tailcall(this.undefine1(fun)).flatMap(
            funv =>
              this
                .trampolineM(this.undefine1, args)
                .map(argsv => AtomlangSwap(atomv, funv, argsv, pos))
          )
      )
    case _ => tailcall(super.undefine1(exp))
  }
}
