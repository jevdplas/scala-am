    ...
    object CompareAndSet extends Primitive {
      ...
      override def call(
        ...
        case (_, v) :: (_, old) :: (_, nw) :: Nil =>
          getPointerAddresses(v).foldLeft(
            ...
            (acc, addr) =>
              for {
                ...
                (bool, eff) <- ifThenElseWithEffs(eqv) {
                  (schemeLattice.bool(true), Effects.wAddr(addr) ++ Effects.rAddr(addr))
                } { (schemeLattice.bool(false), Effects.rAddr(addr)) }
        ...
        getPointerAddresses(v)
          .foldLeft(MayFail.success[(Store[A, V], Effects), Error]((store, Effects.noEff())))(
            (acc, addr) =>
              ...
              } yield (store_.update(addr, atom(value)), effs ++ Effects.wAddr(addr))
          )
          .map { case (store, effs) => (value, store, effs) } // Return value = new value of the atom.
      }
    }
